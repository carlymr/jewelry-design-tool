...
Beyond beads, the schema's `COMPONENT_SHAPES` cover strand components — chain, jump rings, clasps, connectors, cabochons. Component shapes bypass BeadSwatch's filled-silhouette pipeline and draw themselves (`componentElement`); a placed `chain` element always represents a 1-inch segment (`length_mm: 25.4`), which keeps per-element stock counting aligned with chain sold by the inch. Placeable categories are Beads/Cabochons/Findings plus anything with a visual (`PLACEABLE_CATEGORIES` in DesignBoard).
