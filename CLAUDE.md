...
Beyond beads, the schema's `COMPONENT_SHAPES` cover strand components — chain, jump rings, clasps, connectors, cabochons. Component shapes bypass BeadSwatch's filled-silhouette pipeline and draw themselves (`componentElement`); a placed `chain` element always represents a 1-inch segment (`length_mm: 25.4`), which keeps per-element stock counting aligned with chain sold by the inch. Cabochons render as hanging pendants (stone below the string with its long axis vertical) and advance the strand by only `CABOCHON_ADVANCE_MM`, not the stone's size — except by `visual.drill`: `none` hangs from a dashed placeholder bail (needs a setting), `front-back` and legacy/unknown from a solid bail ring, `top` straight from the wire with no ring, and `center` sits inline like a bead and advances by its own length. `DRILL_TYPES`/`DRILL_LABELS` live in `lib/bead-visual.ts`; the inventory edit row exposes the selector for cabochons. Placeable categories are Beads/Cabochons/Findings (`PLACEABLE_CATEGORIES` in DesignBoard); the palette additionally shows any other material that already has a visual.
